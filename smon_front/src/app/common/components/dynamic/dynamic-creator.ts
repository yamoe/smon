...
export class DynamicCreator {
        ...
        this.subscription = rowsComponent.changes.subscribe(t => {
            setTimeout(() => {
                Object.keys(this.waitingCmps).forEach(function (key) {
                    let info = this.waitingCmps[key];
                    let componentClass = info[0];
                    let input = info[1];
                    let output = info[2];

                    if (this.hasRoom()) {
                        this.addComponent(key, componentClass, input, output);
    ...
    public add(key: string, componentClass: Type<any>, input: { [key: string]: any } = undefined, output: { [key: string]: any } = undefined): boolean {
        ...
        if (!this.hasRoom()) {
            this.addRoom();
            this.waitingCmps[key] = [componentClass, input, output];
            return true;
        }
        return this.addComponent(key, componentClass, input, output);
    }
    ...
    private addComponent(key: string, componentClass: Type<any>, input: { [key: string]: any } = undefined, output: { [key: string]: any } = undefined): boolean {
